class QuizElementsHelper {
    ...
    nextBtnHandler() {
      ...
      let result;
      if (!selectedOption) result = this.quiz.skipCurrentQuestion();
      else result = this.quiz.answerCurrentQuestion(selectedOption.value);
  
      if (result.finished || result.timeOver) this.showResultCard(result.result);
      else if (result) this.parseNextQuestion(result.nextQ);
